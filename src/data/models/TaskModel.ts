...
export class TaskModel {
  ...
  static async getTasks(userId: string): Promise<Task[]> {
    ...
    const allRows = await db.getAllAsync<{ id: number; title: string; description: string | null; isCompleted: number; tagId: number | null }>('SELECT * FROM tasks WHERE isDeleted = 0 AND userId = ?', [userId]);
